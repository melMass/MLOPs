...
def run(
    ...
    if input_scheduler["numpy_image"] is not None:
        # Guide Image
        input_image = mlops_image_utils.colors_numpy_array_to_pil(
            input_scheduler["numpy_image"]
        )
        pipeline_call_kwargs["image"] = input_image
        # Mask
        input_mask = mlops_image_utils.colors_numpy_array_to_pil(
            input_scheduler["numpy_mask"]
        )
    ...
    conditional_embeddings = torch.from_numpy(
        numpy.array(
            numpy.array(input_embeddings["conditional_embedding"]).reshape(tensor_shape)
        )
    )

    unconditional_embeddings = torch.from_numpy(
        numpy.array(
            numpy.array(input_embeddings["unconditional_embedding"]).reshape(
                tensor_shape
            )
        )
    )
    ...
    pooled_conditional_embeddings = input_embeddings["pooled_conditional_embedding"][
        : pooled_tensor_shape[0] * pooled_tensor_shape[1]
    ]
    pooled_conditional_embeddings = torch.from_numpy(
        numpy.array(
            numpy.array(pooled_conditional_embeddings).reshape(pooled_tensor_shape)
        )
    )
    pooled_unconditional_embeddings = input_embeddings[
        "pooled_unconditional_embedding"
    ][: pooled_tensor_shape[0] * pooled_tensor_shape[1]]
    pooled_unconditional_embeddings = torch.from_numpy(
        numpy.array(
            numpy.array(pooled_unconditional_embeddings).reshape(pooled_tensor_shape)
        )
    )

    pipeline_call_kwargs["pooled_prompt_embeds"] = pooled_conditional_embeddings
    pipeline_call_kwargs["negative_pooled_prompt_embeds"] = (
        pooled_unconditional_embeddings
    )
    ...
    # Model
    model_path = mlops_utils.ensure_huggingface_model_local(
        model, os.path.join("$MLOPS_MODELS", "diffusers"), cache_only
    )

    # Delete kwargs not used by pipeline
    _keep = inspect.signature(
        pipelines_lookup.pipelines[pipeline_type]
    ).parameters.keys()
    ...
    if tiling != "none":
        ...
        def asymmetricConv2DConvForward(
            self,
            input: torch.Tensor,
            weight: torch.Tensor,
        ...
        for cl in conv_layers:
            if (
                isinstance(cl, diffusers.models.lora.LoRACompatibleConv)
                and cl.lora_layer is None
            ):
    ...
    if lora_weights["model"] != "":
        lora_model_path = mlops_utils.ensure_huggingface_model_local(
            lora_weights["model"],
            os.path.join("$MLOPS_MODELS", "diffusers"),
            cache_only,
            model_type="all",
        )
    ...
    # Delete kwargs not used by pipeline
    _keep = inspect.signature(
        pipelines_lookup.pipelines[pipeline_type].__call__
    ).parameters.keys()
